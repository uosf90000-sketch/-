...
import { designPreferences, mergeRoomDesign } from "@/lib/design-scope";
...
export async function POST(request: Request) {
  ...
  if (!uploadId)
    return Response.json(
      { ok: false, error: "uploadId مطلوب" },
      { status: 400 },
    );
  if (!key)
    return Response.json(
      { ok: false, error: "OPENAI_API_KEY غير مضاف" },
      { status: 503 },
    );
  ...
  try {
    ...
    const analysis =
      input?.bim ||
      JSON.parse(await readFile(path.join(dir, "analysis.json"), "utf8"));
    let roomReading: any = { rooms: [] };
    try {
      roomReading = JSON.parse(
        await readFile(path.join(dir, "rooms.json"), "utf8"),
      );
    } catch {}

    const preferences = designPreferences(input, analysis);
    let previousDesign: any = null;
    try {
      previousDesign = JSON.parse(
        await readFile(path.join(dir, "design.json"), "utf8"),
      )?.design;
    } catch {}

    const scanReady =
      analysis?.scan?.project?.scanStatus === "ready" ||
      analysis?.scanStatus === "ready";
    if (!scanReady)
      return Response.json(
        { ok: false, error: "تحليل BIMy لم يكتمل بعد." },
        { status: 409 },
      );
    ...
    const prompt = [
      preferences.room
        ? "أنت المصمم الداخلي لمنصة بيتي. صمم الغرفة المحددة فقط، ولا تضف عناصر لأي غرفة أخرى."
        : "أنت المصمم الداخلي لمنصة بيتي. صمم المنزل كاملًا اعتمادًا على المخطط الحقيقي ونتيجة BIMy وأسماء الغرف المرسلة.",
      `اتجاه التصميم المطلوب: ${preferences.style}`,
      preferences.room
        ? `الغرفة المستهدفة: ${preferences.roomName}. استخدم هذا الاسم حرفيًا في حقل room. حدودها الهندسية: ${JSON.stringify(preferences.room)}. حوّل إحداثيات الأثاث إلى planX/planY داخل حدود المنزل كما في العارض، واجعل جميع العناصر داخل مضلع هذه الغرفة.`
        : "",
    ...
    if (["jpg", "jpeg", "png", "webp"].includes(meta.extension)) {
      ...
      content.push({
        type: "input_image",
        image_url: `data:${meta.type || "image/jpeg"};base64,${bytes.toString("base64")}`,
      });
    }
    ...
    if (!response.ok)
      return Response.json(
        {
          ok: false,
          error: body?.error?.message || `OpenAI HTTP ${response.status}`,
        },
        { status: 502 },
      );

    const generated = JSON.parse(outputText(body));
    const design = preferences.room
      ? mergeRoomDesign(
          previousDesign,
          generated,
          preferences.room,
    ...
    return Response.json(
      { ok: false, error: error?.message || "فشل التصميم الداخلي" },
      { status: 500 },
    );
  }
}
